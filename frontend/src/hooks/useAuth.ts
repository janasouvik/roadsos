...
export function useAuth(): AuthContextValue {
  ...
  const loadUser = useCallback(async () => {
    ...
    try {
      ...
      // Try refresh
      try {
        const storedRefreshToken = localStorage.getItem('roadsos_refresh_token') || undefined;
        ...
        if (newRefreshToken) {
          localStorage.setItem('roadsos_refresh_token', newRefreshToken);
        }
        ...
        localStorage.removeItem('roadsos_refresh_token');
        setAccessToken(null);
  ...
  const signup = useCallback(async (data: SignupData) => {
    ...
    try {
      ...
      if (refreshToken) {
        localStorage.setItem('roadsos_refresh_token', refreshToken);
      }
