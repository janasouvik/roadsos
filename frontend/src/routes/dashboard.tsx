...
function Dashboard() {
  ...
  const [isGameAccident, setIsGameAccident] = useState(false);
  const [gameTelemetry, setGameTelemetry] = useState<{ speed: string; overtakes: string; distance: string } | null>(null);
  ...
  useEffect(() => {
    if (!isLoading && !user) {
      if (typeof window !== 'undefined') {
        ...
        if (search.includes("accident=true")) {
          sessionStorage.setItem("pending_accident_sos", search);
        }
      }
      navigate({ to: "/login" });
    }
  ...
  const triggerParallelGameSos = async () => {
    const toastId = toast.loading("🚨 Countdown expired! Transmitting automatic Doctor & Police emergency SOS alerts...", { duration: 60000 });
    try {
      const speed = gameTelemetry?.speed || "120";
      const overtakes = gameTelemetry?.overtakes || "0";
      const distance = gameTelemetry?.distance || "0";
      ...
      // Parallel dispatch to Doctor (HOSPITAL) and Police (POLICE)
      const [resHospital, resPolice] = await Promise.all([
        sosApi.create({
          latitude: accidentCoords?.latitude || 12.82739,
          longitude: accidentCoords?.longitude || 77.58999,
      ...
      toast.dismiss(toastId);
      toast.error(error.message || "Failed to broadcast crash SOS alerts.");
    } finally {
      setIsGameAccident(false);
      setGameTelemetry(null);
    }
  };

  // Handle game crash automatic SOS trigger
  useEffect(() => {
    if (!isLoading && user && user.role === 'USER') {
      if (typeof window === 'undefined') return;

      const pendingAccident = sessionStorage.getItem("pending_accident_sos") || window.location.search;
      if (pendingAccident.includes("accident=true")) {
        // Clear immediately to prevent double trigger
        sessionStorage.removeItem("pending_accident_sos");
        
        // Clean URL
        const newUrl = window.location.origin + window.location.pathname;
        window.history.replaceState({}, document.title, newUrl);

        const params = new URLSearchParams(pendingAccident);
        const speed = params.get("speed") || "120";
        const overtakes = params.get("overtakes") || "0";
        const distance = params.get("distance") || "0";

        setGameTelemetry({ speed, overtakes, distance });
        setIsGameAccident(true);
        setAccidentSosTimer(10);
        setAddress("Kaggalipura Road - Car Game Simulator Crash Scene");

        // Lock user GPS/position
        const acquireLocation = async () => {
          ...
          if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
              (pos) => {
                setAccidentCoords({ latitude: pos.coords.latitude, longitude: pos.coords.longitude });
              },
              () => {
        ...
        playBeep();
        toast.error("🚨 Collision Alert Detected from Car Simulator! Initializing countdown.");
      }
    }
  }, [user, isLoading]);
  ...
  }, [accidentSosTimer, accidentCoords, address, isGameAccident, gameTelemetry]);
              ...
              <div className="relative space-y-6">
                ...
                <div>
                  <h2 className="text-2xl font-extrabold text-brand-red uppercase tracking-wider">
                    {isGameAccident ? "🚨 Crash Telemetry Detected!" : "🚨 Admin Dispatching SOS!"}
                  </h2>
                  <p className="text-sm text-muted-foreground mt-2">
                    {isGameAccident 
                      ? "An automated emergency crash SOS broadcast has been initialized by your vehicle simulator."
                ...
                <div className="p-4 rounded-2xl border border-white/5 bg-white/[0.02] text-left text-xs space-y-2">
                  ...
                  {isGameAccident && gameTelemetry && (
                    <div className="pt-2 mt-2 border-t border-white/10 space-y-1.5 font-mono text-[11px]">
                      <div className="flex justify-between text-brand-red font-bold">
                        <span>💥 Impact Speed:</span>
                        <span>{gameTelemetry.speed} KM/H</span>
                      </div>
                      <div className="flex justify-between text-white/90">
                        <span>🏎️ Overtakes prior:</span>
                        <span>{gameTelemetry.overtakes}</span>
                      </div>
                      <div className="flex justify-between text-white/90">
                        <span>📏 Distance:</span>
                        <span>{gameTelemetry.distance} m</span>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <p className="text-[11px] text-brand-red font-semibold animate-pulse">
                  {isGameAccident
                    ? `Broadcasting simulated crash SOS event in ${accidentSosTimer}s...`
