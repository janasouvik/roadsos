...
export function HospitalDashboard() {
  ...
  const [activeSosForMap, setActiveSosForMap] = useState<SosRequest | null>(null);
        ...
        res.data.forEach(req => {
          if (req.status === 'PENDING' && !currentPendingIds.has(req.id)) {
            if (req.severity === 'HIGH' || req.severity === 'CRITICAL') {
              toast.error(`🚨 CRITICAL INJURY ALERT: Patient ${req.user?.fullName || 'Unknown'} - Blood Group: ${req.user?.bloodGroup || 'N/A'}`);
              playAlarmSound();
            } else {
  ...
  const playAlarmSound = () => {
    try {
      ...
      osc.type = 'sawtooth';
      osc.frequency.setValueAtTime(950, audioCtx.currentTime); // High pitch alarm tone
  ...
  const handleUpdateStatus = async (sosId: string, newStatus: string) => {
    try {
      const res = await adminApi.updateSosStatus(sosId, newStatus, "AIIMS Delhi Trauma Ambulance");
      if (res.success) {
  ...
  const handleTriggerSos = async (userId: string) => {
    try {
      const res = await adminApi.triggerSos(userId, {
        ...
        latitude: 12.82739,
        longitude: 77.58999,
        address: "Kaggalipura Road - Accident Warning Dispatched by Hospital Admin",
  ...
  const filteredSos = sosRequests.filter(req => {
    ...
    const matchesSearch = !query || 
      ...
      (req.user?.bloodGroup || "").toLowerCase().includes(query) ||
      (req.address || "").toLowerCase().includes(query);
    
    return matchesStatus && matchesSeverity && matchesSearch;
  });

  // Manage active SOS for map selection
  useEffect(() => {
    if (filteredSos.length > 0) {
      if (!activeSosForMap || !filteredSos.some(r => r.id === activeSosForMap.id)) {
        setActiveSosForMap(filteredSos[0]);
      }
    } else {
      setActiveSosForMap(null);
    }
  }, [sosRequests, statusFilter, severityFilter, searchQuery]);

  const getSeverityBadgeColor = (severity?: string) => {
    switch (severity?.toUpperCase()) {
      case "CRITICAL": return "bg-red-500/20 text-red-400 border border-red-500/30 animate-pulse font-extrabold";
      case "HIGH": return "bg-orange-500/20 text-orange-400 border border-orange-500/30 font-bold";
    ...
    <div className="flex flex-col md:flex-row gap-6 mt-8">
      ...
      <div className="flex-1">
        <GlassCard className="min-h-[500px]">
          ...
          {activeTab === "sos" && (
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <h2 className="text-2xl font-bold">🩺 Medical Desk (Trauma Center)</h2>
              ...
              <div className="flex flex-wrap items-center gap-3 p-3 rounded-2xl border border-white/5 bg-white/[0.02] text-xs">
                ...
                >
                  ...
                  <option value="ACCEPTED" className="bg-[#0f172a]">Ambulance Assigned</option>
                  <option value="IN_PROGRESS" className="bg-[#0f172a]">En-route/Active</option>
                  <option value="COMPLETED" className="bg-[#0f172a]">Admitted/Closed</option>
              ...
              {loading ? (
                ...
                <p className="text-muted-foreground text-center py-10">No trauma cases registered.</p>
              ) : (
                <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-6">
                  {/* Left Column: SOS Incident Cards */}
                  <div className="grid gap-4 max-h-[580px] overflow-y-auto pr-2">
                    {filteredSos.map((req) => {
                      const isCriti = req.severity === 'CRITICAL' || req.severity === 'HIGH';
                      const isActiveMap = activeSosForMap?.id === req.id;
                      return (
                        <div 
                          key={req.id} 
                          ...
                          className={`p-5 rounded-2xl border transition-all cursor-pointer text-left ${
                            isActiveMap
                              ? "border-brand-red bg-brand-red/5 ring-1 ring-brand-red/30 shadow-[0_0_15px_rgba(255,30,45,0.1)]"
                            ...
                            <div className="space-y-3 flex-1">
                              ...
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${getStatusBadgeColor(req.status)}`}>
                                  {req.status === 'ACCEPTED' ? 'AMBULANCE ASSIGNED' : req.status === 'COMPLETED' ? 'PATIENT ADMITTED' : req.status}
                                </span>
                              ...
                              <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
                                ...
                                <div className="flex items-center gap-3">
                                  <div className="px-3.5 py-1.5 rounded-xl bg-red-500/10 border border-red-500/20 flex items-center gap-2 text-xs">
                                  ...
                                  <div className="px-3.5 py-1.5 rounded-xl bg-orange-500/10 border border-orange-500/20 flex items-center gap-2 text-xs">
                              ...
                              <div className="flex items-start gap-2 text-xs">
                                ...
                                <div>
                                  ...
                                  >
                                    Route Ambulance on Google Maps
                                  </a>
                                </div>
                              </div>
                            </div>

                            {/* Quick hospital dispatch actions */}
                            <div className="flex flex-row lg:flex-col items-stretch justify-center gap-2 lg:w-52 shrink-0" onClick={(e) => e.stopPropagation()}>
                              <button
                              ...
                              {req.status === 'PENDING' && (
                                <button
                                  ...
                                  className="flex-1 lg:flex-initial bg-red-600 hover:bg-red-700 text-white px-3 py-2.5 rounded-xl text-xs font-extrabold uppercase tracking-wider inline-flex items-center justify-center gap-1.5 cursor-pointer transition-colors"
                                >
                                  <Activity className="h-3.5 w-3.5" /> Dispatch Ambulance
                                </button>
                              )}
                              ...
                              {req.status === 'ACCEPTED' && (
                                ...
                                >
                                  <Clock className="h-3.5 w-3.5" /> Set En-route
                                </button>
                              )}
                              ...
                              {(req.status === 'PENDING' || req.status === 'ACCEPTED' || req.status === 'IN_PROGRESS') && (
                                ...
                                >
                                  <CheckCircle className="h-3.5 w-3.5" /> Patient Admitted
                                </button>
                              )}
                            </div>
                          </div>
                  ...
                  {/* Right Column: Dynamic Live Map */}
                  <div className="h-[400px] lg:h-[580px] lg:sticky lg:top-6 flex flex-col gap-3">
                    <div className="flex items-center justify-between text-xs text-muted-foreground bg-white/[0.02] border border-white/5 p-3 rounded-2xl">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <MapPin className="h-4 w-4 text-brand-red animate-pulse shrink-0" />
                        <span className="truncate">Patient: <strong className="text-white">{activeSosForMap?.user?.fullName || "Anonymous"}</strong></span>
                      </div>
                      <span className="font-mono bg-white/10 px-2 py-0.5 rounded text-[10px] uppercase font-bold text-white shrink-0">
                        {activeSosForMap?.user?.bloodGroup || "O+"}
                      </span>
                    </div>

                    <FloatingMap
                      title="Patient Location — Ambulance En Route"
                      className="flex-1"
                      userLocation={activeSosForMap ? { latitude: activeSosForMap.latitude, longitude: activeSosForMap.longitude } : null}
                      sosActive={activeSosForMap ? activeSosForMap.status !== 'COMPLETED' : false}
                      highlightNearestType="HOSPITAL"
                      adminLocation={activeSosForMap && activeSosForMap.status !== 'PENDING' ? {
                        latitude: activeSosForMap.latitude + 0.015,
                        longitude: activeSosForMap.longitude - 0.012,
                      } : null}
                      adminLabel="Ambulance Unit"
                    />
                  </div>
                </div>
              )}
          ...
          {activeTab === "users" && (
            <div className="space-y-6">
              ...
              {loading ? (
                ...
                <p className="text-muted-foreground text-center py-10">No registered patient cards.</p>
              ) : (
                    ...
                    <div key={u.id} className="p-4 rounded-xl border border-white/10 bg-white/5 flex flex-col gap-4">
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        ...
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                          ...
                          >
                            {expandedUserId === u.id ? "Hide Details" : "Show Patient Details"}
                          </button>
          ...
          {activeTab === "admins" && (
            <div className="space-y-6">
              <h2 className="text-2xl font-bold">Medical Admins</h2>
              {loading ? (
                <div className="flex justify-center items-center h-40">
                  <Loader2 className="animate-spin h-8 w-8 text-brand-red" />
                </div>
      ...
      {/* PATIENT EMERGENCY HEALTH CARD OVERLAY */}
      <AnimatePresence>
        {selectedSos && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-xl glass-card rounded-3xl p-7 border border-white/10 shadow-2xl text-left overflow-hidden z-10"
            >
              <div className="absolute top-0 right-0 h-32 w-32 bg-red-500/5 rounded-full blur-3xl" />
              
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-3">
                  <div className="h-12 w-12 rounded-2xl bg-red-500/10 border border-red-500/30 flex items-center justify-center text-red-500 font-bold text-xl">
                    +
                  </div>
                  <div>
              ...
              <div className="space-y-4">
                ...
                <div className="pt-2">
                  ...
                  {selectedSos.user?.emergencyContacts && selectedSos.user.emergencyContacts.length > 0 ? (
                    ...
                    <div className="text-muted-foreground italic text-xs">No emergency contacts listed.</div>
                  )}
                </div>
              </div>
            </motion.div>
