...
function Services() {
  ...
  // Location of dispatched admin responder (simulated from nearest matching service)
  const [responderLocation, setResponderLocation] = useState<{ latitude: number; longitude: number } | null>(null);
  const [responderLabel, setResponderLabel] = useState<string>("Responder");
  ...
  const fetchActiveSos = async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        // Compute responder location when SOS is accepted/in-progress
        if (active && (active.status === 'ACCEPTED' || active.status === 'IN_PROGRESS')) {
          // Use a simulated responder location close to the user (offset slightly, ~1-2 km away)
          // In a real app, this would come from the admin's live GPS pushed via WebSocket/API
          const sosLat = active.latitude;
          const sosLng = active.longitude;

          // Simulate responder dispatching from ~1.5km north-east of the incident
          const responderLat = sosLat + 0.013; // ~1.4km north
          const responderLng = sosLng + 0.010; // ~1km east

          setResponderLocation({ latitude: responderLat, longitude: responderLng });

          const typeLabel: Record<string, string> = {
            HOSPITAL: "Ambulance Unit",
            AMBULANCE: "Ambulance Unit",
            POLICE: "Police Patrol",
            TOWING: "Towing Vehicle",
            RESCUE: "Rescue Team",
          };
          setResponderLabel(typeLabel[active.emergencyType] || "Responder");
        } else if (!active) {
          ...
          <FadeUp>
            <GlassCard hover={false}>
              ...
              <div className="grid md:grid-cols-2 gap-5">
                ...
                <FloatingMap 
                  showLegend={false} 
                  title={responderLocation ? "Responder En Route" : ""} 
                  className="h-full min-h-[320px]" 
                  sosActive={!!activeSos}
                  userLocation={activeSos ? { latitude: activeSos.latitude, longitude: activeSos.longitude } : coords}
                  adminLocation={responderLocation}
                  adminLabel={responderLabel}
                />
              </div>
            </GlassCard>
          </FadeUp>
