                      ...
                      return (
                        <div 
                          ...
                          className={`p-5 rounded-2xl border transition-all cursor-pointer text-left ${
                            isActiveMap
                              ? "border-brand-blue bg-brand-blue/5 ring-1 ring-brand-blue/30 shadow-[0_0_15px_rgba(37,99,255,0.1)]"
                        ...
                        >
                          <div className="flex flex-col lg:flex-row justify-between gap-4">
                            ...
                            <div className="flex flex-row lg:flex-col items-stretch justify-center gap-2 lg:w-48 shrink-0" onClick={(e) => e.stopPropagation()}>
                              <button
                  ...
                  <div className="h-[400px] lg:h-[580px] lg:sticky lg:top-6 flex flex-col gap-3">
                    <div className="flex items-center justify-between text-xs text-muted-foreground bg-white/[0.02] border border-white/5 p-3 rounded-2xl">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <MapPin className="h-4 w-4 text-brand-blue animate-pulse shrink-0" />
                        <span className="truncate">Driver: <strong className="text-white">{activeSosForMap?.user?.fullName || "Anonymous"}</strong></span>
                      </div>
                      <span className="font-mono bg-white/10 px-2 py-0.5 rounded text-[10px] uppercase font-bold text-white shrink-0">
                        {activeSosForMap?.user?.vehicleType || "Vehicle"}
                      </span>
                    </div>

                    <FloatingMap
                      title="Incident Location — Patrol En Route"
                      className="flex-1"
                      ...
                      highlightNearestType="POLICE"
                      adminLocation={activeSosForMap && activeSosForMap.status !== 'PENDING' ? {
                        latitude: activeSosForMap.latitude - 0.011,
                        longitude: activeSosForMap.longitude + 0.014,
                      } : null}
                      adminLabel="Police Patrol"
                    />
                  </div>
                </div>
              )}
